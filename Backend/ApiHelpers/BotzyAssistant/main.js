...
export async function BotzyAssistant(message, source, id){
    ...
    try {
        ...
        let call = await ai.generateText(sys, usr, { structuredOutput: PromptsAndSchemas.triage.schema })
        if(call.isErr()){
            throw new Error(`Ahh bloody typical, the clanker has thrown an error. Lets see... it says : ${call.value}`);
        }
...
const PromptsAndSchemas = {
    triage: {
        sys:'GENERAL TONE: You are a helpful aussie personal assistant called Botzy Bee. Interact with the user in a casual style. '+
    'At all times you should be laid back but never seek to flatter. You should be honest, grounded and unbiased. '+
    ...
    'A holding statement should be something like "Ok, Ill just go and check that for you" or "Ive asked one of my team to do XYZ for you. Ill get back to you with an update."', 
        usr:(task, actions) => { return `Here is the user's message : ${task} \n \n Here are the available actions ${actions}`},
        schema: {
            "type": "object",
            "properties": {
                "action": {
