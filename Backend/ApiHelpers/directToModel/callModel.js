 ...
 */
export async function directToModel(
  userQuery,
  aiSettings,
  webGrounding,
  socketID
  ...
  try {
    ...
    let systemMessage =
      'You are a professional AI assistant. Answer the users query to the best of your ability' +
    ...
    if (webGrounding == false) {
      ...
      emitToSocket({
        socketId: socketID,
        event: 'direct_to_model_response',
        data: call,
      });
    } else {
      ...
      emitToSocket({
        sockedId: socketID,
        event: 'direct_to_model_response',
        data: { outcome: 'Ok', value: call.value.text },
      });
    }
    if (call.isErr()) {
    ...
    emitToSocket({
      sockedId: socketID,
      event: 'direct_to_model_response',
      data: { outcome: 'Error', value: error },
    });
    return;
  }
}
