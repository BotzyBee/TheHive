...
import { QuickAskAgent } from '../services/QuickAsk/index.js';
...
export async function handleQAMessage(frontendMessage, socketId = null) {
  ...
  if (processedMsg.isErr()) {
    return Services.v2Core.Helpers.Err(
      `Error : could not process the messages into classes. ${processedMsg.value}`
    );
  }
  ...
  if (id == null) {
    ...
    if (newJob.isErr()) {
      return Services.v2Core.Helpers.Err(
        `Error : (createQuickAskJob) - ${newJob.value}`
      );
    }
    ...
    if (job.isErr()) {
      return Services.v2Core.Helpers.Err(
        `Error : (handleQAMessage) - ${job.value}`
      );
    }
    let processedMsg = processApiMessagesToClasses(frontendMessage.messages);
    if (processedMsg.isErr()) {
      return Services.v2Core.Helpers.Err(
        `Error (handleQAMessage 2) : could not process the messages into classes. ${processedMsg.value}`
      );
    }
    // Handle Stop
    ...
    if (job.value.isRunning == true) {
      return Services.v2Core.Helpers.Err(
        `Job ${id} is still running. Cannot add new instructions unless stopped or complete.`
      );
    }

    // add new messages to messageHistory
    processedMsg.value.forEach((msg) =>
      job.value.messageHistory.addMessage(msg)
    );
