...
export async function handleTAMessage(frontendMessage, socketId = null) {
  ...
  if (id == null) {
    ...
    if (processedMsg.isErr()) {
      return Services.v2Core.Helpers.Err(
        `Error (handleTAMessage) : could not process the messages into classes. ${processedMsg.value}`
      );
    }
    ...
    if (newJob.isErr()) {
      return Services.v2Core.Helpers.Err(
        `Error : (handleTAMessage) - ${newJob.value}`
      );
    }
    ...
    if (job.isErr()) {
      return Services.v2Core.Helpers.Err(
        `Error : (handleTAMessage) - ${job.value}`
      );
    }
    let processedMsg = processApiMessagesToClasses(frontendMessage.messages);
    if (processedMsg.isErr()) {
      return Services.v2Core.Helpers.Err(
        `Error (handleTAMessage 2) : could not process the messages into classes. ${processedMsg.value}`
      );
    }
    // Handle Stop
