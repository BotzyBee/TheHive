...
class AI_JOB_MANAGER {
  ...
  addNewJob(aiJobClass) {
    ...
    return Services.v2Core.Helpers.Err(
      'Error (AI_JOB_MANAGER -> addNewJob) : input must be an instance of AiJob or SubClass of it.'
    );
  }
  ...
  async checkNonAllocated() {
    ...
    if (jobsAwaiting == 0) {
      return Services.v2Core.Helpers.Ok('No Jobs To Allocate');
    }
    ...
    for (let i in tempJobs) {
      jobArray.push(this.allocateAndProgress(this.#workerThreads, tempJobs[i]));
    }
    ...
    if (errorList.length != 0) {
      ...
      return Services.v2Core.Helpers.Err(
        `Error (checkNonAllocated -> allocateAndProgress ) : ${JSON.stringify(errorList)}`
      );
    }
    this.#allocatingJobs = false;
  ...
  async allocateAndProgress(workerThreads = false, jobID) {
    ...
    if (jobID == undefined) {
      return Services.v2Core.Helpers.Err(
        'Error (allocateAndProgress) - JobID missing from input params'
      );
    }
    ...
    if (fetchCall.isErr()) {
      return Services.v2Core.Helpers.Err(
        `Error (allocateAndProgress -> jobListManager) : ${fetchCall.value}`
      );
    }
    /**@type {AiJob} */
    ...
    Services.v2Core.Helpers.log(
      `Progressing ${jobClassObject.agentType} - ${jobID}`
    );
    if (workerThreads == false) {
      ...
      if (processCall.isErr()) {
        ...
        return Services.v2Core.Helpers.Err(
          `Error (allocateAndProgress -> run( ${jobClassObject.id} ) ) ${processCall.value}`
        );
      }
    } else {
      ...
      if (
        Services.v2Core.WorkerPool.threads?.length == 0 ||
        Services.v2Core.WorkerPool.closed
      ) {
        return Services.v2Core.Helpers.Err(
          'Error (allocateAndProgress) : Worker Pool is offline!'
        );
      }
      ...
      //processCall ERROR
      if (processCall.outcome == 'Error') {
        // cant use .isErr() as pool is stringifying the result!
        ...
        return Err(
          `Error (allocateAndProgress -> pool run() ) ${processCall.value.errorText}`
        );
      }
  ...
  jobListManager(params = {}) {
    ...
    if (insertJob) {
      if (this.AI_JOBS.some((job) => job.id === insertJob.id)) {
        return Err(
          `Error (jobListManager): Record ${insertJob.id} already exists.`
        );
      }
    ...
    if (getJob) {
      ...
      return job
        ? Services.v2Core.Helpers.Ok(job)
        : Err(`Error (jobListManager): Could not find ID: ${getJob}`);
    }
    ...
    if (replaceJob) {
      ...
      return Services.v2Core.Helpers.Ok(
        `ID ${replaceJob.id} has been replaced/added`
      );
    }
    ...
    if (prune) {
      ...
      this.AI_JOBS = this.AI_JOBS.filter((job) => {
        if (job.status == Status.Complete && job.endEpochMs < oneHourAgo) {
          Services.v2Core.Helpers.log(
            `Pruning Job ID ${job.id} from AI_JOBS list.`
          );
    ...
    return Services.v2Core.Helpers.Err(
      'Error (jobListManager): No valid operation provided.'
    );
  }
