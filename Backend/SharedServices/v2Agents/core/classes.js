...
export class MessageLog {
  ...
  getAllMessagesString() {
    return this.allMessages.map((msg) => JSON.stringify(msg)).join('\n\n');
  }
  ...
  getSimpleUserAgentComms() {
    return this.allMessages
      .filter(
        (msg) =>
          msg?.type === 'text' &&
          (msg?.role === Roles.Agent || msg?.role === Roles.User)
      )
  ...
  getToolMessagesAsObject() {
    let rtnObject = {};
    let allToolMsgs = this.allMessages.filter(
      (msg) => msg?.role === Roles.Tool
    );
    ...
    switch (this.whoGetsUpdates) {
      ...
      case 'BotzyVoice':
        botzyMemory.push({
          type: 'jobUpdate',
          id: this.id,
          message: `[ Job Update ] : ${statusMessage}`,
        });
        break;
    }
  }
  ...
  emitFinalResult() {
    switch (this.whoGetsUpdates) {
      ...
      case 'BotzyVoice':
        ...
        let filtered = this.taskOutput.filter(
          (message) => message.type == 'text' || message.type == 'data'
        );
        //
        botzyMemory.push({
          type: 'jobOutcome',
    ...
    Object.keys(data).forEach((key) => {
      ...
      if (Object.prototype.hasOwnProperty.call(this, key)) {
        if (key == 'status') {
          ...
          let msgs = Services.aiAgents.processApiMessagesToClasses(
            data[key].allMessages
          ).value;
