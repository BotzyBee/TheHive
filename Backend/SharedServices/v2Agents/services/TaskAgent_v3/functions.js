...
export async function loadingMain(agentObject) {
  ...
  if (preLoad) {
    ...
    for (let i = 0; i < urls.length ?? 0; i++) {
      ...
      const targetURL = Services.aiAgents.ToolHelpers.pathHelper.join(
        root,
        urls[i].trim()
      );
      let fileInfo =
        Services.fileSystem.CRUD.getFileExtensionAndSize(targetURL); // returns : {"extension":"xlsx","sizeBytes":8665,"sizeFormatted":"8.46 KB"}}
      if (fileInfo.isErr()) {
        // Flow to healing
        ...
        agentObject.errors.push(
          `Error (loadingMain -> getFileExtensionAndSize ) : ${fileInfo.value}`
        );
        return;
      }
      // lookup correct read tool
      let strategyToUse = Services.fileSystem.IO.fileRegistry.getByExt(
      ...
      if (strategyToUse.read == null) {
        ...
        agentObject.TaskState.handoverData = [];
        agentObject.errors.push(
          `Error (loadingMain -> getByExt ) : No strategy to read ${fileInfo.value.extension} files.`
        );
        return;
      }
      ...
      if (toolCall.isErr()) {
        ...
        agentObject.TaskState.handoverData = [];
        agentObject.errors.push(
          `Error (loadingMain -> strategyToUse.readFN() : ${toolCall.value})`
        );
        return;
      }
...
export async function createPlan(agentObject) {
  // Plan or Re-plan
  const isReplan =
    agentObject.TaskState.currentFlowState == TaskFlow.Plan.rePlan
      ? true
      : false;
  const statusLabel = isReplan
    ? 'Updating Task Plan...'
    : 'Creating Task Plan...';
  ...
  // [][]  -------------------------- [][]
  const completedActions = isReplan
    ? agentObject.getCompletedActionsSummary()
    : 'No prior actions completed.';
  ...
  const actionStep = await agentObject.generateText(systemPrompt, userPrompt, {
  ...
  if (actionStep.isErr()) {
    ...
    agentObject.setFlowState(TaskFlow.Healing.main);
    agentObject.TaskState.handoverMessage =
      'Planning Step 1 - has thrown an error. Consider re-trying planning phase';
  ...
  if (actionResponse == undefined) {
    ...
    agentObject.setFlowState(TaskFlow.Healing.main);
    agentObject.TaskState.handoverMessage =
      'Planning Step has thrown an error - Action Response is undefined. Consider re-trying planning phase.';
    agentObject.TaskState.handoverData = [];
    agentObject.errors.push(
      'Planning Step has thrown an error - Action Response is undefined.'
    );
    return;
  }
  if (actionResponse.status === 'need info') {
    ...
    agentObject.TaskState.handoverMessage =
      'Task Agent (Planning Phase) is requesting information from the user.';
    agentObject.TaskState.handoverData = [actionResponse.failText];
    return;
  }
  if (
    actionResponse.status === 'cant plan' ||
    actionResponse?.plan.length == 0
  ) {
    // Flow to Message User
    agentObject.setFlowState(TaskFlow.Action.messageUser);
    agentObject.TaskState.handoverMessage =
      'Task Agent (Planning Phase) could not complete an action plan.';
    agentObject.TaskState.handoverData = [actionResponse.failText];
    return;
  }
  ...
  let Step1PlanMerged = actionStep.value.plan
    .map((item) => item.action)
    .join('\n');
  let tools = await Services.database.Helpers.getToolsOrGuidesForTask(
    `${agentObject.task} \n\n ${Step1PlanMerged}`,
    15
  );
  if (tools.isErr()) {
    ...
    agentObject.setFlowState(TaskFlow.Healing.main);
    agentObject.TaskState.handoverMessage =
      'Planning Step 2 - has thrown an error. Consider re-trying planning phase';
  ...
  tools.value.push(
    {
      ToolName: 'rePlanTool',
      ToolDescription:
        'This tool forces the agent to re-plan, updating the plan to include tool data or information which was not originally available.' +
    ...
    },
    {
      ...
      ToolDescription:
        'This tool returns the data or outcome of this task to the user. Every plan should end with this tool.',
      FilePath: 'TA_TOOL',
    }
  );
  ...
  const newActions = toolStep.value.plan.map(
    (p) => new TaskAction(p.action, p.tool)
  );
  // If updating, keep completed actions and append/replace the rest
  const completedOldActions = oldPlan.filter((p) => p.complete);
  agentObject.plan = isReplan
    ? [...completedOldActions, ...newActions]
    : newActions;
  ...
  agentObject.plansNeedApproved = agentObject.task
    .toLowerCase()
    .includes('!!plans-need-approved');
  if (agentObject.plansNeedApproved) {
    // Flow to Message User
    agentObject.setFlowState(TaskFlow.Action.messageUser);
    agentObject.TaskState.handoverMessage =
      'The user needs to approve the plan before we continue.';
    ...
    agentObject.TaskState.handoverMessage =
      'Planning step completed - continue to tool calling.';
    agentObject.TaskState.handoverData = [];
  }
  agentObject.maxLoops =
    (agentObject.maxLoopBuffer + agentObject.plan.length) * 3;
  return;
}
...
export async function performNextAction(agentObject) {
  ...
  if (nextAction == null) {
    ...
    agentObject.TaskState.handoverMessage =
      'No actions needing complete - process final output.';
    agentObject.TaskState.handoverData = [];
    return;
  }
  ...
  if (nextAction.tool == 'rePlanTool') {
    ...
    agentObject.TaskState.handoverMessage =
      'Re-Planning triggered by rePlanTool';
    agentObject.TaskState.handoverData = [];
    // Add tool to message history
    const msg = new Services.aiAgents.Classes.TextMessage({
      role: Services.aiAgents.Constants.Roles.Tool,
      textData: 'Re-planning tool has been called...',
      toolName: 'rePlanTool',
      instructions: agentObject.phaseMessage,
      metadata: { actionID: nextAction.id },
    });
  ...
  if (nextAction.tool == 'returnToUser') {
    ...
    // Add tool to message history
    let msgText =
      'Final tool - The return to user tool was called. ' +
    ...
    const msg = new Services.aiAgents.Classes.TextMessage({
      ...
      textData: msgText,
      toolName: 'returnToUser',
      instructions: 'Return the output to user.',
      metadata: { actionID: nextAction.id },
    });
    agentObject.messageHistory.addMessage(msg);
    return;
  ...
  if (toolObj.isErr()) {
    ...
    agentObject.setFlowState(TaskFlow.Healing.main);
    agentObject.TaskState.handoverMessage =
      'Fetch Tool Object has thrown an error. Consider re-trying tool-call phase';
  ...
  let craftedParams = await agentObject.generateText(
    ...
    ),
    {
      ...agentObject.aiSettings,
      structuredOutput:
        Services.aiAgents.InputParse.parserPrompts.craftParams.schema,
    }
  ...
  if (craftedParams.isErr()) {
    ...
    agentObject.setFlowState(TaskFlow.Healing.main);
    agentObject.TaskState.handoverMessage =
      'Craft Parameters Action has thrown an error. Consider re-trying tool-call phase';
  ...
  let resolvedParams = Services.aiAgents.InputParse.parseNunjucksTemplate(
    craftedParams.value.params,
    fullContext
  );
  if (resolvedParams.isErr()) {
    ...
    agentObject.setFlowState(TaskFlow.Healing.main);
    agentObject.TaskState.handoverMessage =
      'Build Parameter object step has thrown an error. Consider re-trying tool-call phase';
    ...
    agentObject.debugParams.push({
      tool: nextAction.tool,
      paramsCrafted: craftedParams.value.params,
      paramsResolved: resolvedParams.value,
      toolInputSchema: toolObj.value.details.inputSchema,
    });
    return;
  }
  ...
  if (toolCall.isErr()) {
    ...
    agentObject.setFlowState(TaskFlow.Healing.main);
    agentObject.TaskState.handoverMessage =
      'Tool Calling action has thrown an error. Consider re-running the tool calling phase or re-plan is the tool used is incorrect.';
...
export async function messageUser(agentObject) {
  ...
  if (returnMessage.isErr()) {
    ...
    agentObject.setFlowState(TaskFlow.Healing.main);
    agentObject.TaskState.handoverMessage =
      'Attempt to messaage the user failed. Consider trying again.';
...
export async function reviewUserMessage(agentObject) {
  ...
  let processUserMessage = await agentObject.generateText(
    ...
    PromptsAndSchemas.reviewUserMsg.usr(
      convoHistory,
      JSON.stringify(agentObject.plan)
    ),
    {
      ...agentObject.aiSettings,
      structuredOutput: PromptsAndSchemas.reviewUserMsg.schema,
    }
  );
  agentObject.healPrompt = '';
  if (processUserMessage.isErr()) {
    ...
    agentObject.setFlowState(TaskFlow.Healing.main);
    agentObject.TaskState.handoverMessage =
      'Reviewing User Messaage Failed. Consider trying user message review again.';
  ...
  if (processUserMessage.value?.action === undefined) {
    ...
    agentObject.setFlowState(TaskFlow.Healing.main);
    agentObject.TaskState.handoverMessage =
      'Reviewing User Messaage Failed. The output did not follow the schema - action field missing.';
    agentObject.TaskState.handoverData = [];
    agentObject.errors.push(
      'Error (reviewUserMessage) - Reviewing User Messaage Failed. The output did not follow the schema - action field missing.'
    );
    return;
  }
  ...
  if (processUserMessage.value.action == 'update_task_plan') {
    ...
    if (newTaskWording.isErr()) {
      ...
      agentObject.setFlowState(TaskFlow.Healing.main);
      agentObject.TaskState.handoverMessage =
        'Failed when creating new task wording. Consider retrying process user message.';
      agentObject.TaskState.handoverData = [];
      agentObject.errors.push(
        `Error (reviewUserMessage) - Failed when creating new task wording. ${newTaskWording.value}`
      );
      return;
    }
  ...
  agentObject.setFlowState(TaskFlow.Healing.main);
  agentObject.TaskState.handoverMessage =
    'Error: reviewAndReturn -> Catch new message from User : User message could not be parsed into a known output.';
  agentObject.TaskState.handoverData = [];
  agentObject.errors.push(
    'Error: reviewAndReturn -> Catch new message from User : User message could not be parsed into a known output.'
  );
...
export async function reviewToolOutput(agentObject) {
  ...
  if (toolActionID == null) {
    ...
    agentObject.setFlowState(TaskFlow.Healing.main);
    agentObject.TaskState.handoverMessage =
      'Error : Review tool output. No ActionID found. Consider re-running action phase or route to final output.';
    agentObject.TaskState.handoverData = [];
    agentObject.errors.push(
      'Error : Review tool output. No ActionID found. Consider re-running action phase'
    );
    return;
  }
  ...
  if (actionObj == null) {
    ...
    agentObject.setFlowState(TaskFlow.Healing.main);
    agentObject.TaskState.handoverMessage =
      'Error : Review phase - actionObject returned null - consider re-running planning phase.';
    agentObject.TaskState.handoverData = [];
    agentObject.errors.push(
      'Error (reviewAndReturn -> Get Action & Tool Objects ) : Action Object returned null'
    );
    return;
  }
  ...
  let processedToolMessages =
    Services.aiAgents.AgentSharedServices.stripOutAudioAndImageData(toolOutput);
  if (processedToolMessages.isErr()) {
    // Flow to healing
    agentObject.setFlowState(TaskFlow.Healing.main);
    agentObject.TaskState.handoverMessage =
      'Error : Review phase - Process Tool Messages Failed. Consider re-running review stage.';
    agentObject.TaskState.handoverData = [];
    agentObject.errors.push(
      `Error (reviewAndReturn -> stripOutAudioAndImageData ) : ${processedToolMessages.value}`
    );
    return;
  }
  ...
  let completeCheck = await agentObject.generateText(
    ...
    {
      ...agentObject.aiSettings,
      structuredOutput: PromptsAndSchemas.completeCheck.schema,
    }
  ...
  if (completeCheck.isErr()) {
    ...
    agentObject.setFlowState(TaskFlow.Healing.main);
    agentObject.TaskState.handoverMessage =
      'Error : Review phase - Tool review call has failed. Consider re-calling review stage.';
    agentObject.TaskState.handoverData = [];
    agentObject.errors.push(
      `Error (reviewAndReturn -> generateText ) : ${completeCheck.value}`
    );
    return;
  }
  ...
  if (completeCheck.value?.status == undefined) {
    ...
    agentObject.TaskState.handoverData = [];
    agentObject.errors.push(
      `Error (reviewAndReturn -> generateText (Schema Fail!) ) : status value is undefined.}`
    );
    return;
  }
  console.log(
    'REVIEW :: ' + JSON.stringify(completeCheck.value.status, null, 2)
  );
  ...
  if (completeCheck.value.status == 'INCOMPLETE') {
    ...
    // plan needs updated
    if (
      completeCheck.value.replan === true ||
      completeCheck.value.replan === 'true'
    ) {
      // Flow to Replan
      agentObject.setFlowState(TaskFlow.Plan.rePlan);
    ...
    if (ac >= agentObject.toolRetryCount) {
      ...
      agentObject.setFlowState(TaskFlow.Plan.rePlan);
      agentObject.TaskState.handoverMessage =
        `Tool: ${actionObj.tool}  ID: ${toolActionID} - Has failed multiple times when attempting to complete the action.` +
    ...
    agentObject.setActionText(
      toolActionID,
      `${actionObj.action}. Feedback from last attempt: ${fb}`
    );
    agentObject.setFlowState(TaskFlow.Plan.rePlan);
    agentObject.TaskState.handoverMessage = '';
    agentObject.TaskState.handoverData = [];
  ...
  if (completeCheck.value.status == 'COMPLETE') {
    ...
    let toolActionID =
      agentObject.TaskState.handoverData[0]?.metadata?.actionID || null;

    agentObject.incrimentActionCount(toolActionID);
    agentObject.setActionComplete(toolActionID);
    // Push data to context & message history
    processedToolOp.value.rawMessages.forEach((msg) =>
      agentObject.messageHistory.addMessage(msg)
    );
...
export async function processFinalOutput(agentObject) {
  ...
  let output = await Services.aiAgents.AgentSharedServices.finialiseOutput(
    agentObject,
    saveFolder
  ); // Finalises from context
  if (output.isErr()) {
    // Flow to healing
    agentObject.setFlowState(TaskFlow.Healing.main);
    agentObject.TaskState.handoverMessage =
      'Process Final Output has thrown an error. Consider re-running final output processing.';
    agentObject.TaskState.handoverData = [];
    agentObject.errors.push(
      `Error (processFinalOutput -> finialiseOutput ) : ${output.value}`
    );
    return;
  }
...
export async function healing(agentObject) {
  ...
  const nextStep = await agentObject.generateText(
    ...
    {
      ...agentObject.aiSettings,
      structuredOutput: PromptsAndSchemas.healError.schema,
      quality: 3,
    }
