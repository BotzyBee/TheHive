...
export function addAnyDirectData(inputString, dataObject) {
  ...
  if (typeof templateString !== 'string') {
    return Services.v2Core.Helpers.Err(
      `Error (addAnyDirectData) : templateString must be a string`
    );
  }
  if (
    ...
    return Services.v2Core.Helpers.Err(
      `Error (addAnyDirectData) : dataObject must be an object`
    );
  }
  ...
  try {
    ...
    return Services.v2Core.Helpers.Err(
      `Error (addAnyDirectData) : ${error.message}`
    );
  }
}
...
export function buildObject(entries, depth = 0, maxDepth = 10) {
  // 1. Safety Checks
  if (depth > maxDepth)
    return { outcome: 'Error', message: `Max depth ${maxDepth} exceeded.` };
  if (!Array.isArray(entries))
    return { outcome: 'Error', message: `Input is not an array.` };
