...
export async function processMessageForContext(
  ...
  try {
    ...
    return Services.v2Core.Helpers.Err(
      `Error : (processMessageForContext) - Error performing stringify : ${error}`
    );
  }
  ...
  if (dataSize > summaryDataSizeThreshold) {
    let summary = await shortenLargeValues(
      messageObject,
    ...
    if (summary.isErr()) {
      return Services.v2Core.Helpers.Err(
        `Error (processMessageForContext -> shortenLargeValues) : ${summary.value}`
      );
    }
...
async function shortenLargeValues(
  data,
  maxSize,
  aiOptions = {},
  visited = new WeakSet()
  ...
  if (
    data === null ||
    ['undefined', 'boolean', 'symbol', 'bigint', 'function'].includes(
      typeof data
    )
  ) {
  ...
  if (typeof data === 'object') {
    if (visited.has(data))
      return Services.v2Core.Helpers.Ok('[Circular Reference]');
    visited.add(data);
  }
    ...
    if (strVal.length > maxSize) {
      ...
      if (base64Check === true) {
        return Services.v2Core.Helpers.Ok(
          `[ Base64 data. Length: ${strVal.length} ]`
        );
      }
      // Else create summary
      ...
      return summary.isErr()
        ? summary
        : Services.v2Core.Helpers.Ok(summary.value);
    }
    return Services.v2Core.Helpers.Ok(data);
  }
  ...
  if (data instanceof Uint8Array || data instanceof ArrayBuffer) {
    return data.byteLength > maxSize
      ? Services.v2Core.Helpers.Ok(
          `[Large Binary Data: ${data.byteLength} bytes]`
        )
  ...
  if (Array.isArray(data)) {
    ...
    const results = await Promise.all(
      data.map((item) =>
        shortenLargeValues(item, maxSize, (aiOptions = {}), visited)
      )
    );
  ...
  if (typeof data === 'object') {
    ...
    for (const [key, value] of Object.entries(data)) {
      const res = await shortenLargeValues(
        value,
        maxSize,
        (aiOptions = {}),
        visited
...
export async function createSummary(stringRes, aiOptions = {}) {
  ...
  if (dataSummary.isErr()) {
    return Services.v2Core.Helpers.Err(
      `Error (curateToolData) : ${dataSummary.value}`
    );
  }
...
export async function finialiseOutput(agentObject, saveFolder) {
  ...
  if (agentObject == null) {
    return Services.v2Core.Helpers.Err(
      `Error (finialiseOutput) : agentObject is missing or null`
    );
  }
  ...
  let outputOverview = await agentObject.aiCall.generateText(
    ...
    ),
    {
      ...
      structuredOutput: PromptsAndSchemas.outputOverview.schema,
    }
  ); // { outputPlan: [ {type: Enum, instructions: string, contextKey}... ] }
  agentObject.addAiCount(1);
  if (outputOverview.isErr()) {
    return Services.v2Core.Helpers.Err(
      `Error (finialiseOutput -> outputOverview ) : ${outputOverview.value}`
    );
  }
  let outputPlan = outputOverview.value.outputPlan || [];
  if (outputPlan.length == 0) {
    return Services.v2Core.Helpers.Err(
      `Error (finialiseOutput -> outputOverview ) : Returned empty output plan!`
    );
  }
  ...
  for (let i = 0; i < outputPlan.length; i++) {
    if (outputPlan[i].type === 'Text') {
      let txt = await processText(
        agentObject,
        outputPlan[i]?.contextKey || null
      );
    ...
    if (outputPlan[i].type === 'Save') {
      let check = agentObject.messageHistory.getMessagesById(
        outputPlan[i]?.contextKey
      );
      if (check == null) {
        return Services.v2Core.Helpers.Err(
          `Error: (finialiseOutput -> save) - Could not located any data for ${outputPlan[i]?.contextKey}`
        );
      }
      let save = await Services.aiAgents.AgentHelpers.saveMessageContent(
        check,
        saveFolder,
        null
      );
...
async function processText(agentObject, contextKey = null) {
  ...
  let formatCall = await agentObject.aiCall.generateText(
    ...
    {
      ...agentObject.aiSettings,
      structuredOutput: PromptsAndSchemas.processText.schema,
    }
  ); // { output: [enum "Text_Output", "Quote_Text" ], data: string }
  if (formatCall.isErr()) {
    return Services.v2Core.Helpers.Err(
      `Error ( processText ) : ${formatCall.value}`
    );
  }
  agentObject.addAiCount(1);
  ...
  if (
    formatCall.value?.output === undefined ||
    formatCall.value?.data === undefined
  ) {
    return Services.v2Core.Helpers.Err(
      `Error: (processText) - AI agent has not returned 'output' and/or 'data' key. Unable to progress.`
    );
  }
  ...
  if (formatCall.value.output == 'Quote_Text') {
    ...
    let augmentedTextOutput = addAnyDirectData(formatCall.value.data, {
      context: contextObj,
    });
    if (augmentedTextOutput.isErr()) {
      return Services.v2Core.Helpers.Err(
        `Error: (processText) : ${augmentedTextOutput.value}`
      );
    }
  ...
  let msg = new TextMessage({
    role: Roles.Agent,
    textData: opText,
    mimeType: contextObj?.mime || 'text/plain',
  });
  agentObject.messageHistory.addMessage(msg);
...
async function processImage(agentObject, contextKey) {
  if (contextKey == null) {
    return Services.v2Core.Helpers.Err(
      `Error: (processImage) - contextKey is missing`
    );
  }
  let check = agentObject.messageHistory.getMessagesById(contextKey);
  if (check == null) {
    return Services.v2Core.Helpers.Err(
      `Error: (processImage) - Could not located any data for ${contextKey}`
    );
  }
...
async function processAudio(agentObject, contextKey) {
  if (contextKey == null) {
    return Services.v2Core.Helpers.Err(
      `Error: (processAudio) - contextKey is missing`
    );
  }
  let check = agentObject.messageHistory.getMessagesById(contextKey);
  if (check == null) {
    return Services.v2Core.Helpers.Err(
      `Error: (processAudio) - Could not located any data for ${contextKey}`
    );
  }
  // Clone & strip out unnessessary data.
...
async function processData(agentObject, contextKey) {
  if (contextKey == null) {
    return Services.v2Core.Helpers.Err(
      `Error: (processData) - contextKey is missing`
    );
  }
  ...
  if (check == null) {
    return Services.v2Core.Helpers.Err(
      `Error: (processData) - Could not locate any data for ${contextKey}`
    );
  }
...
export function stripOutAudioAndImageData(messageArray) {
  ...
  if (!Array.isArray(messageArray) || messageArray.length == 0) {
    return Services.v2Core.Helpers.Err(
      `Error: (stripOutAudioAndImageData) - messageArray is not an array or is empty!`
    );
  }
  ...
  for (let i = 0; i < opMessages.length; i++) {
    if (opMessages[i].type === 'image' || opMessages[i].type === 'audio') {
      opMessages[i].base64 =
        `[ Base 64 data removed due to size - Full data can be found in message ${messageArray[i].id} ]`;
    }
  }
  return Services.v2Core.Helpers.Ok(opMessages);
}
...
Make sure not to save data that has already been saved as part of the plan.`;
    ...
    schema: {
      description:
        "A schema defining the execution plan for an agent's multi-modal output.",
      type: 'object',
      properties: {
        outputPlan: {
          type: 'array',
          ...
          items: {
            ...
            properties: {
              type: {
                ...
                description:
                  'The format or action type of the output component.',
              },
              instructions: {
                type: 'string',
                description:
                  'Concise instructions to help the next AI Agent to include the correct data. This is not for the user!',
              },
              contextKey: {
                type: 'string',
                description:
                  'The key relating to any data to be added (optional).',
              },
            },
