    ...
    try {
      ...
      if (this.messageHistory.getMessageCount() > 1) {
        ...
        if (taskCall.isErr()) {
          ...
          this.errors.push(
            `Error (Quick Task Agent -> startTask -> generateText) : ${taskCall.value}`
          );
          this.emitFailed();
      ...
      // Get tool list
      this.status.setCustomStatus(
        'Determining best tool to use for the task...'
      );
      ...
      if (tools.isErr()) {
        ...
        this.emitUpdateStatus('Quick Ask Agent has failed :(');
        this.errors.push(
          `Error (Quick Task Agent -> startTask -> getToolsOrGuidesForTask) : ${tools.value}`
        );
        this.emitFailed();
        return Services.v2Core.Helpers.Err(
          `Error (Quick Task Agent -> startTask -> getToolsOrGuidesForTask) : ${tools.value}`
      ...
      let routingCall = await this.aiCall.generateText(
        ...
        PromptsAndSchemas.routingCall.usr(
          this.task,
          JSON.stringify(tools.value)
        ),
        {
          ...this.aiSettings,
          structuredOutput: PromptsAndSchemas.routingCall.schema,
        }
      );
      ...
      if (routingCall.isErr()) {
        ...
        this.isRunning = false;
        this.errors.push(
          `Error (Quick Task Agent -> startTask -> aiCall) : ${routingCall.value}`
        );
        this.emitFailed();
        return Services.v2Core.Helpers.Err(
          `Error (Quick Task Agent -> startTask -> aiCall) : ${routingCall.value}`
      ...
      if (
        routingCall.value.nextAction == 'no-suitable-tool' ||
        ...
        let msg = new Services.aiAgents.Classes.TextMessage({
          ...
          textData: routingCall.value.message,
        });
      ...
      // Get tool details
      if (!this.isRunning)
        return Services.v2Core.Helpers.Ok('Job stopped by user.');
      let toolDetails = await Services.database.Helpers.getToolDetails(
        routingCall.value.toolName
      );
      if (toolDetails.isErr()) {
        this.setFailed();
        this.isRunning = false;
        this.errors.push(
          `Error (Quick Task Agent -> startTask -> getToolDetails) : ${toolDetails.value}`
        );
        this.emitFailed();
        return Services.v2Core.Helpers.Err(
          `Error (Quick Task Agent -> startTask -> getToolDetails) : ${toolDetails.value}`
      ...
      for (let i = 0; i < this.toolRetryCount; i++) {
        ...
        let paramObject = Services.aiAgents.InputParse.parseNunjucksTemplate(
          paramsCall.value.params,
          fullContext
        );
        ...
        this.emitUpdateStatus(
          `Calling tool: ${toolDetails.value.details.toolName} ...`
        );
        ...
        this.status.setCustomStatus(
          `Calling ${toolDetails.value.details.toolName} ...`
        );
      ...
      if (failedCount >= this.toolRetryCount) {
        ...
        this.emitFailed();
        return Services.v2Core.Helpers.Err(
          `Error ( Quick Task Agent (Param/ Tool loop) ) : ${JSON.stringify(loopErrors)}`
        );
      }
      ...
      for (let i = 0; i < newMessageLen; i++) {
        ...
        if (toolCall.value[i].role === Services.aiAgents.Constants.Roles.Tool) {
          ...
          if (processed.isErr()) {
            ...
            this.emitUpdateStatus('Quick Ask Agent has failed :(');
            this.errors.push(
              `Error : (Quick Task Agent -> startTask -> processMessageForContext ) : ${processed.value}`
            );
            this.emitFailed();
            return Services.v2Core.Helpers.Err(
              `Error : (Quick Task Agent -> startTask -> processMessageForContext ) : ${processed.value}`
      ...
      let formattedOP =
        await Services.aiAgents.AgentSharedServices.finialiseOutput(
          this,
          'UserFiles/BotzysFiles/QuickAskOutputs'
        );
      if (formattedOP.isErr()) {
        ...
        this.emitUpdateStatus('Quick Ask Agent has failed :(');
        this.errors.push(
          `Error (Quick Task Agent -> startTask -> finialiseOutput ) : ${formattedOP.value}`
        );
        this.emitFailed();
...
If you need to clarify the task or sent the user another message then output nextAction: "clarify-task" and message: [a suitable, polite, professional and helpful message to the user.]`,
    usr: (task, tools) => {
      return `Here is the user task <task> ${task} </task>. Here are the available tools <tools> ${tools} </tools>`;
    },
    schema: {
      type: 'object',
      properties: {
        nextAction: {
          type: 'string',
          enum: ['no-suitable-tool', 'clarify-task', 'use-tool'],
        ...
        message: {
          type: 'string',
          description:
            'A descriptive message to the user or a clear task for the tool to follow.',
        },
      },
      required: ['nextAction', 'toolName', 'message'],
      additionalProperties: false,
    },
  },
  newTask: {
    sys:
      'You are the Task Synchronisation Specialist, an expert analyst tasked with distilling the dialogue' +
    ...
    usr: (messages) => {
      return `Here is the conversation between user and agent: ${messages}`;
    },
  },
};
