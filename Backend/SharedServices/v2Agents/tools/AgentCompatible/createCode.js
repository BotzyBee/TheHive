...
export const details = {
  ...
  creator: 'Botzy Bee',
  overview:
    'This tool uses AI to create, edit, or review code. \n' +
  ...
  guide: `There are 3 different tasks that this tool can perform. What parameters you use will depend on the task.
...
If the user provided any context or guides they can be included via 'contextOrGuides' parameter.`,
  inputSchema: {
    ...
    properties: {
      taskDescription: {
        type: 'string',
        description: 'The coding task, request, or question.',
      },
      codeForProcessing: {
        type: 'string',
        description:
          'The specific code needing edited. Not needed if code generation or code review is requested.',
      },
      contextOrGuides: {
        type: 'string',
        description:
          'Reference context - this can be text or other code which will help the agent.',
      },
    },
...
export async function run(Shared, params = {}, agent = {}) {
  ...
  const routerSys =
    "You are a routing agent for a coding tool. Based on the user's task description and whether context is provided, determine the mode of operation:\n" +
  ...
  const routerCall = await aiCall.generateCode(
    routerSys,
    `Task: ${taskDescription}`,
    { ...aiSettings, structuredOutput: routerSchema }
  );
  if (routerCall.isErr())
    return Shared.v2Core.Helpers.Err(
      `Error (createCodeTool -> Router phase) : ${routerCall.value}`
    );
  aiCount++;
  ...
  if (mode === 'REVIEW') {
    ...
    const reviewSys =
      "You are an expert Senior Developer. Review the provided code based on the user's task description. " +
    ...
    if (reviewCall.isErr())
      return Shared.v2Core.Helpers.Err(
        `Error (createCodeTool -> Review phase) : ${reviewCall.value}`
      );
    aiCount++;
    retAR.push(
      new Shared.aiAgents.Classes.TextMessage({
  ...
  if (mode === 'EDIT') {
    ...
    if (!codeForProcessing || codeForProcessing === '') {
      return Shared.v2Core.Helpers.Err(
        'Error: EDIT mode selected by router, but no existing code was provided in the context.'
      );
    }

    // 1. Generate an Edit Plan
    const editPlanSys =
      "You are a Senior Developer. Review the existing code and the user's edit request. " +
          ...
          items: {
            ...
            properties: {
              instruction: {
                type: 'string',
                description:
                  "Specific instruction for the editor, e.g., 'Replace the calculateTotal function with the provided code' or 'Insert the provided imports at the top of the file'.",
              },
              codeSnippet: {
                type: 'string',
                description:
                  'The raw code snippet to be inserted or swapped in. Provide ONLY the raw code.',
              },
            },
          },
        },
    ...
    if (editPlanCall.isErr())
      return Shared.v2Core.Helpers.Err(
        `Error (createCodeTool -> Edit Plan phase) : ${editPlanCall.value}`
      );
    ...
    for (const [index, edit] of edits.entries()) {
      safeEmit(
        agent,
        `Coding Tool 🖥️ :: Applying edit - ${index + 1} of ${edits.length}...`
      );
      ...
      if (editResult.isErr())
        return Shared.v2Core.Helpers.Err(
          `Error (createCodeTool -> SuperEditor Edit phase) : ${editResult.value}`
        );
  ...
  if (mode === 'CREATE') {
    ...
    const archSys =
      'You are a Software Architect. Create a valid file SKELETON. ' +
        ...
        workOrders: {
          ...
          items: {
            ...
            properties: {
              ...
              description: {
                type: 'string',
                description: 'Detailed logic requirements for this marker.',
              },
            },
          },
        },
    ...
    const archCall = await aiCall.generateCode(archSys, taskDescription, {
      ...aiSettings,
      structuredOutput: archSchema,
    });
    if (archCall.isErr())
      return Shared.v2Core.Helpers.Err(
        `Error (createCodeTool -> Architecture phase) : Architecture phase failed. ${archCall.value}`
      );
    aiCount++;
    ...
    for (const [index, order] of orders.entries()) {
      safeEmit(
        agent,
        `Coding Tool 🖥️ :: Generating code for marker (${index + 1} of ${orders.length})...`
      );
      const devSys =
        `You are a Senior Developer. Write ONLY the code required for ${order.marker}. ` +
      ...
      const devCall = await aiCall.generateCode(
        devSys,
        'Output ONLY the raw code logic. No markdown. No preamble.',
        aiSettings
      );
      if (devCall.isErr())
        return Shared.v2Core.Helpers.Err(
          `Error (createCodeTool -> Development phase) : ${devCall.value}`
        );
      aiCount++;
      ...
      if (editResult.isErr())
        return Shared.v2Core.Helpers.Err(
          `Error (createCodeTool -> SuperEditor phase) : ${editResult.value}`
        );
  ...
  return Shared.v2Core.Helpers.Err(
    'Error: Router failed to select a valid mode.'
  );
}
