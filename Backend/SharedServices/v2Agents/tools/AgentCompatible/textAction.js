...
export const details = {
  ...
  creator: 'Botzy Bee',
  overview:
    'This tool uses AI to create text, summarise text, extract information or modify a given text input. \n' +
...
export async function run(Shared, params = {}, agent = {}) {
  ...
  if (taskDescription == null) {
    return Shared.v2Core.Helpers.Err(
      `Error (AiTextAction Tool) - Input taskDescription missing or null.`
    );
  }
  ...
  const sysText =
    'You are a tool which extracts, summarises, modifies a given text input. Focus on quality and accuracy. Use UK English.' +
  ...
  let call = await aiCall.generateText(sysText, usrText, {
    ...aiSettings,
    structuredOutput: {
      type: 'object',
      ...
      properties: {
        ...
        ext: {
          type: 'string',
          description:
            "The file extension associated with your text output. Default is 'txt' or 'md' - but could be 'js', 'html' etc if required.",
        },
      },
  ...
  if (call.isErr()) {
    return Shared.v2Core.Helpers.Err(
      `Error (AiTextAction -> Generate Text) : ${call.value}`
    );
  }
