...
export const details = {
  ...
  creator: 'Botzy Bee',
  overview:
    'This tool uses AI to create or modify images.  \n' +
        ...
        properties: {
          imageOptions: {
            ...
            properties: {
              ...
              contextImage: {
                oneOf: [{ $ref: '#/$defs/ImageMessage' }, { type: 'object' }],
                description:
                  'Image to be edited or used as part of the process',
              },
            },
          },
        },
...
export async function run(Shared, params = {}, agent = {}) {
  ...
  if (contentMessage == null) {
    return Shared.v2Core.Helpers.Err(
      `Error (aiImageTool) - Input contentMessage missing or null.`
    );
  }
  ...
  let call = await aiCall.generateImage(contentMessage, {
    ...aiSettings,
    ...options,
  });
  if (call.isErr()) {
    return Shared.v2Core.Helpers.Err(
      `Error (aiImageTool -> Generate Image) : ${call.value}`
    );
  }
  // Process Outputs
