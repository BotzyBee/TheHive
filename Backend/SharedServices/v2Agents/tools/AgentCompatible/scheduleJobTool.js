...
export const details = {
  ...
  creator: 'Botzy Bee',
  overview:
    'Manages scheduled tasks within the system. Supports three modes: 1 (Schedule an AI call), 2 (Cancel a scheduled task by ID), and 3 (List all active tasks). Limitations: AI callbacks run in the background and their text results are outputted to system logs rather than returned immediately to the user.',
  guide:
    "Always invoke with an explicit 'mode'. For Mode 1, provide a unique timerName, aiComments, and scheduling timing. If the user specifies a relative time ('in 5 minutes'), use 'delayMs'. If the user specifies an absolute time ('at 21:00 on 15/10/24'), calculate and provide the exact ISO 8601 string for 'targetTime'. For Mode 2, provide the timerID. For Mode 3, no additional parameters are required. Add 'intervalMs' only if a recurring task is requested.",
  inputSchema: JSON.stringify({
    ...
    properties: {
      mode: {
        type: 'integer',
        description:
          'Operational mode: 1 = Schedule an AI call, 2 = Cancel a task, 3 = List all timers.',
        enum: [1, 2, 3],
      },
      timerName: {
        type: 'string',
      ...
      delayMs: {
        type: 'integer',
        description:
          'Mode 1: The delay in milliseconds before the first execution (for relative times).',
      },
      targetTime: {
        type: 'string',
        description:
          'Mode 1: A specific Date/Time for execution (ISO 8601 string, for absolute times).',
      },
      intervalMs: {
        type: 'integer',
        description:
          'Mode 1: Optional repeat interval in milliseconds. If omitted, the task runs only once.',
      },
      aiComments: {
        type: 'string',
        description:
          'Mode 1: The task description or instructions for the AI to execute.',
      },
      timerID: {
        type: 'string',
...
async function handleScheduleMode(Shared, params) {
  ...
  if (!timerName) {
    return Shared.v2Core.Helpers.Err(
      "Mode 1 requires 'timerName' to uniquely identify the schedule."
    );
  }
  if (!aiComments) {
    return Shared.v2Core.Helpers.Err(
      "Mode 1 requires 'aiComments' defining the AI instructions."
    );
  }
  ...
  if (targetTime) {
    ...
    if (!Shared.aiAgents.ToolHelpers.isValidHelper(parsedDate)) {
      return Shared.v2Core.Helpers.Err(
        `Invalid targetTime format provided: ${targetTime}. Must be a valid ISO 8601 string.`
      );
    }
    ...
    const now = new Date();
    calculatedDelay =
      Shared.aiAgents.ToolHelpers.differenceInMillisecondsHelper(
        parsedDate,
        now
      );
  ...
  const timerResult = Shared.v2Core.Timers.addNewTimer(
    timerName,
    callbackFn,
    config
  );
  ...
  if (timerResult.isErr()) {
    return Shared.v2Core.Helpers.Err(
      `Failed to create schedule: ${timerResult.value}`
    );
  }
...
async function handleCancelMode(Shared, params) {
  ...
  console.log(
    `Attempting to cancel timer with ID: ${timerID} or Name: ${timerName}`
  );
  ...
  if (cancelResult.isErr()) {
    return Shared.v2Core.Helpers.Err(
      `Failed to cancel task: ${cancelResult.value}`
    );
  }
...
async function handleListMode(Shared) {
  ...
  if (listResult.isErr()) {
    return Shared.v2Core.Helpers.Err(
      `Failed to list tasks: ${listResult.value}`
    );
  }
...
export async function run(Shared, params = {}) {
  ...
  switch (mode) {
    ...
    default:
      return Shared.v2Core.Helpers.Err(
        `Error (scheduleJobTool) : Invalid mode '${mode}'. Supported modes are 1, 2, or 3.`
      );
  }
  ...
  let instructions =
    mode === 1
      ? 'AI task has been scheduled as requested.'
      : mode === 2
        ? 'Scheduled task cancellation has been completed.'
