...
export const details = {
  ...
  creator: 'HiveDev_AI',
  overview:
    'Combines two or more plain-text strings into a single output using a configurable separator (e.g. space, newline, comma). ' +
  ...
  inputSchema: JSON.stringify({
    ...
    properties: {
      texts: {
        type: 'array',
        description:
          'An ordered array of text strings to combine. Must contain at least two non-empty strings.',
        items: {
          type: 'string',
          minLength: 1,
        },
      ...
      separator: {
        type: 'string',
        description:
          'The string inserted between each piece of text. Defaults to a single space if omitted. ' +
...
function validateTexts(texts) {
  ...
  for (let index = 0; index < texts.length; index++) {
    ...
    if (typeof entry !== 'string') {
      return {
        valid: false,
        reason: `Item at index ${index} is not a string (received: ${typeof entry}).`,
      };
    }
    ...
    if (entry.trim().length === 0) {
      return {
        valid: false,
        reason: `Item at index ${index} is empty or whitespace-only.`,
      };
    }
  }
...
export async function run(Shared, params = {}) {
  ...
  if (!validation.valid) {
    return Shared.v2Core.Helpers.Err(
      `[${details.toolName}] Invalid input — ${validation.reason}`
    );
  }
  ...
  try {
    ...
    const message = new Shared.aiAgents.Classes.DataMessage({
      ...
      instructions:
        'Combined the provided text strings using the specified separator. ',
    });
    ...
    return Shared.v2Core.Helpers.Err(
      `[${details.toolName}] Unexpected error: ${error.message}`
    );
  }
}
