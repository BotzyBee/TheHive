...
export const details = {
  ...
  creator: 'Botzy Bee',
  overview:
    'Searches for a specified file or folder and returns any file paths (urls) that contain the search term. Can only search for a single term at a time. \n' +
...
export async function run(Shared, params = {}, agent = {}) {
  ...
  if (searchTerm == null) {
    return Shared.v2Core.Helpers.Err(
      `Error (findFileorFolder) : Params missing or incorrect. Param needed: searchTerm`
    );
  }
  let userFolder = Shared.fileSystem.Constants.userFilesDir;
  let allFoldersAndFiles = await Shared.fileSystem.CRUD.scanFolderRecursively(
    `/${userFolder}`
  );
  if (allFoldersAndFiles.isErr()) {
    return Shared.v2Core.Helpers.Err(
      `Error (findFileorFolder -> scanFolderRecursively) : ${allFoldersAndFiles.value}`
    );
  }
