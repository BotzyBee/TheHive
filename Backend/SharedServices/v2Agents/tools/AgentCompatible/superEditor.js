...
export const details = {
  ...
  creator: 'TheHive',
  overview:
    "An advanced text editor capable of precise inline replacements, targeted insertions, and full document generation. Outputs structural diffs. Limitations: Cannot read external files or perform internet research. Operates strictly on the provided 'document' string.",
  ...
  inputSchema: JSON.stringify({
    ...
    properties: {
      prompt: {
        type: 'string',
        description:
          'The specific instructions detailing the required edits or content generation.',
      },
      document: {
        type: 'string',
        description:
          'The full, current document text. Pass an empty string for new documents.',
      },
      context: {
        type: 'string',
        description:
          'Optional background information or reference material to guide the edits.',
      },
    },
...
export async function run(Shared, params = {}, agent = {}) {
  ...
  if (!prompt || typeof document !== 'string') {
    return Shared.v2Core.Helpers.Err(
      'SuperEditor: Missing prompt or document.'
    );
  }
  ...
  try {
    ...
    for (let i = 0; i < chunks.length; i++) {
      safeEmit(
        agent,
        `SuperEditor - Editing chunk ${i + 1}/${chunks.length} 🐝`
      );
...
---
      ...
      processedChunks.push(
        mutationResult.success ? mutationResult.value : currentChunk
      );
    }
    ...
    return Shared.v2Core.Helpers.Err(
      `Fatal error in SuperEditor: ${error.message}`
    );
  }
}
