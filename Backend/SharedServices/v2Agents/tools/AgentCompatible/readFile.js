...
export const details = {
  ...
  creator: 'Botzy Bee',
  overview:
    'Reads a file on the host filesystem and returns the file contents. The tool can return base64 or utf-8 formats. \n' +
  ...
  guide:
    'If the user provides a URL in their instructions you should use it exactly as provided. Never include /data/ at the start of a URL - this is automatically added later.',
  inputSchema: {
    ...
    properties: {
      filePath: {
        description:
          'The URL path where the file is located. This must include the file name and extension. ',
        type: 'string',
      },
    },
    required: ['filePath'],
...
export async function run(Shared, params = {}) {
  ...
  if (filePath == null) {
    return Shared.v2Core.Helpers.Err(
      `Error (ReadFile Tool) - Input filePath missing or null.`
    );
  }
  ...
  const targetURL = Shared.aiAgents.ToolHelpers.pathHelper.join(
    root,
    decodedPath.trim()
  );
  ...
  if (fileInfo.isErr()) {
    return Shared.v2Core.Helpers.Err(
      `Error (readFile -> getFileExtensionAndSize) : ${fileInfo.value}`
    );
  }
  // lookup correct read tool
  let strategyToUse = Shared.fileSystem.IO.fileRegistry.getByExt(
    fileInfo.value.extension
  ).strategy;
  if (strategyToUse.read == null) {
    return Shared.v2Core.Helpers.Err(
      `Error (readFile) : No strategy to read ${fileInfo.value.extension} files.`
    );
  }
  ...
  if (toolCall.isErr()) {
    return Shared.v2Core.Helpers.Err(
      `Error (readFile -> strategyToUse.readFN() : ${toolCall.value})`
    );
  }
