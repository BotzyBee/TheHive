...
export const details = {
  ...
  creator: 'Botzy Bee',
  overview:
    'A tool for finding and replacing letters, words or phrases within a text string. ' +
  ...
  inputSchema: {
    ...
    required: ['context', 'matchPhrase', 'replaceWith'],
...
export async function run(Shared, params = {}) {
  ...
  if (typeof context === 'object') {
    return Shared.v2Core.Helpers.Err(
      `Error (findAndReplaceTextTool) : Object passed as context - this should only be a string!`
    );
  }
  ...
  try {
    ...
    return Shared.v2Core.Helpers.Err(
      `Error (findAndReplaceTextTool): ${error.message || error}`
    );
  }
}
