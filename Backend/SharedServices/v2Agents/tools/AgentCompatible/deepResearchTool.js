...
export const details = {
  ...
  creator: 'Botzy Bee',
  overview:
    'Conducts deep, factual, and actionable research on a given topic. It scopes the research, deconstructs terms, ' +
  ...
  guide:
    "Provide as much detail as possible in the 'topic' input. The tool will handle the rest, but clearer input can lead to better results. The JobID can be found in global context data.",
  inputSchema: {
    type: 'object',
    properties: {
      questions: {
      ...
      breadth: {
        type: 'number',
        description:
          'Number of top questions to consider for research. Defaults to 1.',
        minimum: 1,
        default: 1,
      },
      savePath: {
        type: 'string',
        description:
          "Relative path to save the final report. Defaults to 'UserFiles/CompletedResearch/'",
      },
      scopeSources: {
        type: 'boolean',
        default: true,
        description:
          'If true, the tool will perform additional searches to locate the best sources.',
      },
    },
    required: ['questions', 'breadth', 'savePath', 'scopeSources'],
...
export class ResearchChunk {
  ...
  async getInitialResearch(domains = []) {
    ...
    if (call.isErr()) {
      return this.Shared.v2Core.Helpers.Err(
        `Error in ResearchChunk -> getInitialResearch : ${call.value}`
      );
    }
    stats.aiCount += 1;
  ...
  async factCheckClaims(domains = []) {
    ...
    if (call.isErr()) {
      return this.Shared.v2Core.Helpers.Err(
        `Error in ResearchChunk -> factCheckClaims : ${call.value}`
      );
    }
    stats.aiCount += 1;
    ...
    console.log(
      'Claims identified for fact-checking: ',
      this.factsOrAssumptions.length
    );
    ...
    for (let fact of this.factsOrAssumptions) {
      ...
      if (searchCall.isErr()) {
        return this.Shared.v2Core.Helpers.Err(
          `Error in ResearchChunk -> factCheckClaims -> aiWebSearch : ${searchCall.value}`
        );
      }
      stats.aiCount += 1;
      ...
      if (scoring.isErr()) {
        return this.Shared.v2Core.Helpers.Err(
          `Error in ResearchChunk -> factCheckClaims -> generateText : ${scoring.value}`
        );
      }
      stats.aiCount += 1;
  ...
  async finaliseResearchChunk() {
    let aiCall = await this.Shared.callAI.aiFactory();
    let clarifications = this.factsOrAssumptions
      .map((f) => f.getSummary())
      .join(`\n \n`);
...
Focus on factual accuracy, actionable information, and clarity. Keep all references intact and ensure the final output is concise and informative.`;
    ...
    if (finalCall.isErr()) {
      return this.Shared.v2Core.Helpers.Err(
        `Error in ResearchChunk -> finaliseResearchChunk -> generateText : ${finalCall.value}`
      );
    }
    stats.aiCount += 1;
...
export async function scopeTopic(
  Shared,
  topic,
  breadth = 5,
  aiSettings,
  ...
  if (termscall.isErr()) {
    return Shared.v2Core.Helpers.Err(
      `Error in scopeTopic -> initTerms : ${termscall.value}`
    );
  }
  // Merge learnings into a single string to pass to the next prompt
  let defs = termscall.value.learnings
    .map((l) => `${l.term} : ${l.definition}`)
    .join(`\n \n`);
  ...
  if (questionCall.isErr()) {
    return Shared.v2Core.Helpers.Err(
      `Error in scopeTopic -> explore : ${questionCall.value}`
    );
  }
  ...
  if (rankedCall.isErr()) {
    return Shared.v2Core.Helpers.Err(
      `Error in scopeTopic -> rankQuestions : ${rankedCall.value}`
    );
  }

  // Combine questions and ratings to select the top questions for research.
  let combined = questionCall.value.questions.map((q, i) => ({
    question: q,
    rating: rankedCall.value.ratings[i],
  }));
  ...
  if (scopeSources == true) {
    sourcesCall = await callAI.webSearch(
      ...
      PromptsAndSchemas.getSources.usr(
        topic,
        topQuestions,
        JSON.stringify(INGORE_DOMAINS)
      ),
      { ...aiSettings, structuredOutput: PromptsAndSchemas.getSources.schema }
    ); // domains: [ 'domain1.com', 'domain2.com', ... ]
    if (sourcesCall.isErr()) {
      return Shared.v2Core.Helpers.Err(
        `Error in scopeTopic -> getSources : ${sourcesCall.value}`
      );
    }
    ...
    sourcesCall2 = await callAI.webSearch(
      ...
      PromptsAndSchemas.getSources.usr(
        topic,
        topQuestions,
        sourcesCall.value.domains.join(', ')
      ),
      { ...aiSettings, structuredOutput: PromptsAndSchemas.getSources.schema }
    ); // domains: [ 'domain1.com', 'domain2.com', ... ]
    if (sourcesCall2.isErr()) {
      return Shared.v2Core.Helpers.Err(
        `Error in scopeTopic -> getSources : ${sourcesCall2.value}`
      );
    }
    stats.aiCount += 2;
  }
  ...
  return Shared.v2Core.Helpers.Ok({
    ...
    sources: scopeSources ? sourcesCall2?.value?.domains : [],
  });
}
...
export async function run(Shared, params = {}, agent = {}) {
  ...
  const {
    questions,
    breadth = 1,
    savePath = 'UserFiles/CompletedResearch/',
    jobID,
    scopeSources = true,
  } = params;
  ...
  for (let i = 0; i < questionLength; i++) {
    ...
    safeEmit(
      agent,
      `Conducting research for question ${i + 1}/${questionLength} - 🤖🐝`
    );
    ...
    let save = await saveTool.run(Shared, {
...
function processReferences(text, referencesArray) {
  if (!text || !referencesArray)
    return { updatedText: text, referenceList: '' };
  ...
  const sortedOldRefs = Array.from(oldRefToNewRef.keys()).sort(
  ...
  const eol =
    typeof process !== 'undefined' && process.platform === 'win32'
      ? '\r\n'
      : '\n';
  const sortedRefs = Array.from(urlToRefNumber.entries()).sort(
    (a, b) => a[1] - b[1]
  );
...
const PromptsAndSchemas = {
  initTerms: {
    ...
    usr: (prompt) => {
      return `Here is the research prompt given by the user <prompt>${prompt}</prompt>`;
    },
    schema: {
      type: 'object',
      description:
        "An object containing a 'learnings' property, where 'learnings' is an array of objects.",
      properties: {
        learnings: {
          type: 'array',
          items: {
  ...
  explore: {
    ...
    usr: (prompt, defs) => {
      return `What questions should I ask to get a better understanding of the following topic? <topic>${prompt}</topic>. Here are some definitions (if any) which might help <definitions> ${defs} </definitions>`;
    },
    schema: {
      type: 'object',
      description:
        "An object containing a 'learnings' property, where 'learnings' is an array of strings (questions)",
      properties: {
        questions: {
          type: 'array',
          items: { type: 'string' },
          description:
            'An array of questions to help the user research a topic.',
        },
      },
      required: ['questions'],
    },
  ...
  rankQuestions: {
    ...
    usr: (prompt, questions) => {
      return `Research topic <topic>${prompt}</topic>. Questions - <questions> ${questions} </questions>`;
    },
    schema: {
      type: 'object',
      description:
        "An object containing a 'ratings' property, where 'ratings' is an array of numbers",
      properties: {
        ratings: {
          type: 'array',
          items: { type: 'number' },
  ...
  getSources: {
    ...
    usr: (prompt, questions, ignore) => {
      return `Find the most reliable sources to complete the following research: <topic>${prompt}</topic> <questions> ${questions} </questions>. Do not output or use the following domains <ignore> ${ignore} </ignore>`;
    },
    schema: {
      type: 'object',
      description:
        "An object containing a 'domains' property, where 'domains' is an array of strings",
      properties: {
        domains: {
          type: 'array',
          items: { type: 'string' },
  ...
  askQuestion: {
    ...
    usr: (question) => {
      return `Here is the research question - <questions> ${question} </questions>.`;
    },
  },
  writeSection: {
...
Avoid repetition as a priority.`,
    usr: (plan, research, report) => {
      return `Here is the plan for the section you need to write <plan> ${plan} </plan>. Here is the research you can use to write this section <research> ${research} </research>. Here is the current version of the report <report> ${report} </report>.`;
    },
  },
...
General statements or opinions should not be included unless they contain a specific claim that can be verified. `,
    usr: (text) => {
      return `Fact-check the following text and return an array of claims or assumptions that should be verified. <text> ${text} </text>`;
    },
    schema: {
      type: 'object',
      description:
        "An object containing a 'claims' property, where 'claims' is an array of strings.",
      properties: {
        claims: {
          type: 'array',
          items: {
          ...
          description:
            'An array of claims or assumptions which should be verified.',
        },
      },
      required: ['claims'],
    },
  ...
  factCheck2: {
    ...
    usr: (claim, research) => {
      return `Here is the claim to fact-check <claim> ${claim} </claim>. Here is the research gathered on the claim <research> ${research} </research>. `;
    },
    schema: {
      type: 'object',
      properties: {
        ...
        clarification: {
          description:
            'Clarification on the claim to help give the agent an understanding of why the claim is supported or refuted and how the fact should be presented in the final output.',
          type: 'string',
        },
      },
...
, ... etc ]`,
    usr: (latestPlan) => {
      return `Here is the latest version of the plan <plan>${latestPlan}</plan>. Output an array which includes the headings and sub-headings. Include as much detail as possible in each array item.`;
    },
    schema: {
      type: 'object',
      properties: {
...
IMPORTANT: If no edits are needed output an empty array [].`,
    usr: (report) => {
      return `Here is the final draft of the report <report>${report}</report>. Really focus on the heading structure and numbering!`;
    },
    schema: {
      type: 'object',
      properties: {
