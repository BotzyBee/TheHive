...
export const details = {
  ...
  creator: 'Botzy Bee',
  overview:
    'This tool uses AI to create a audio message (base64) wav format from user provided text.  \n' +
...
export async function run(Shared, params = {}, agent = {}) {
  ...
  if (contentMessage == null) {
    return Shared.v2Core.Helpers.Err(
      `Error (textToSpeech) - Input contentMessage missing or null.`
    );
  }
  ...
  if (call.isErr()) {
    return Shared.v2Core.Helpers.Err(
      `Error (textToSpeech -> aiCall) : ${call.value}`
    );
  }
