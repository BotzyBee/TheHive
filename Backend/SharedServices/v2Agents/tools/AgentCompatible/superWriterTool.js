...
export const details = {
    ...
    inputSchema: JSON.stringify({
        "type": "object",
        "properties": {
            "taskDescription": { "type": "string", "description": "The writing task, request, or edit instruction." },
            "initialDocument": { "type": "string", "description": "The starting text document. Leave empty if generating from scratch." },
            "contextOrGuides": { 
                "anyOf": [
                    { "type": "string" },
                    { "type": "array", "items": { "anyOf": [{ "type": "string" }, { "type": "object" }] } }
                ],
                "description": "Reference context, facts, or stylistic guidelines. Can be a string, or an array of strings/objects." 
            }
        },
...
export async function run(Shared, params = {}, agent = {}) {
    try {
        ...
        if (!taskDescription || typeof taskDescription !== "string" || taskDescription.trim() === "") {
            return Shared.v2Core.Helpers.Err("Validation Error: 'taskDescription' is required and must be a non-empty string.");
        }
        ...
        if (Array.isArray(contextOrGuides)) {
            contexts = contextOrGuides.map(c => typeof c === 'string' ? c : JSON.stringify(c));
        } else if (contextOrGuides) {
            contexts = [typeof contextOrGuides === 'string' ? contextOrGuides : JSON.stringify(contextOrGuides)];
        }
        ...
        for (let i = 0; i < contexts.length; i++) {
            ...
            if (i === 0 && (!workingDocument || workingDocument.trim() === "")) {
                ...
                const draftSystem = "You are an expert copywriter and technical author. Generate a comprehensive base draft focusing on core structure and content. Do not output markdown code blocks unless requested.";
                const draftUser = `Task: ${taskDescription}\n\nContext/Guides:\n${currentContext}`;
                
                const draftCall = await aiFactory.generateText(draftSystem, draftUser, aiSettings);
                
                if (draftCall.isErr()) {
                    return Shared.v2Core.Helpers.Err(`Error (superWriterTool -> Initial Draft) : ${draftCall.value}`);
                }
            ...
            safeEmit(agent, `Super Writer 📝 : Analysing document to create an iterative edit plan (Context ${i + 1} of ${contexts.length})...`);
                    ...
                    "edits": {
                        ...
                        "items": {
                            ...
                            "properties": {
                                "instruction": { 
                                    "type": "string", 
                                    "description": "Specific instruction for the editor, e.g., 'rewrite the introduction to be more formal' or 'insert factual context about X'." 
                                }
                            },
            ...
            if (edits.length > 0) {
                ...
                for (const [index, edit] of edits.entries()) {
                    safeEmit(agent, `Super Writer 📝 : Applying edit ${index + 1} of ${edits.length} (Context ${i + 1})...`);
                    ...
                    if (editResult.isErr()) {
                        return Shared.v2Core.Helpers.Err(`Error (superWriterTool -> SuperEditor Edit ${index + 1}) : ${editResult.value}`);
                    }
                    ...
                    if (!editData || typeof editData.editedDocument !== "string") {
                        return Shared.v2Core.Helpers.Err(`Error (superWriterTool) : Unexpected malformed response from superEditor on iteration ${index + 1}.`);
                    }

                    workingDocument = editData.editedDocument;
                }
            } else {
                safeEmit(agent, `Super Writer 📝 : Document meets requirements. No further edits needed for context ${i + 1}.`);
            }
        }
        ...
        const finalMessage = new Shared.aiAgents.Classes.TextMessage({
            ...
            instructions: 'Iterative writing and editing complete. Present the finalised markdown text to the user.'
        });
