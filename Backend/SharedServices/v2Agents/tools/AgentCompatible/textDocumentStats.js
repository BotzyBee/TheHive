...
export const details = {
  ...
  creator: 'Botzy Bee',
  overview:
    'A tool which returns the following information on the target text document: \n' +
...
export async function run(Shared, params = {}) {
  ...
  if (filePath == null) {
    return Shared.v2Core.Helpers.Err(
      `Error (readFile) : Params missing or incorrect. Params: should include filePath.`
    );
  }
  ...
  try {
    ...
    fileExtension = Shared.aiAgents.ToolHelpers.pathHelper
      .extname(targetURL)
      .toLowerCase()
      .substring(1);
    ...
    if (textFileExtensions.includes(fileExtension)) {
      ...
      const rl = Shared.aiAgents.ToolHelpers.readlineHelper.createInterface({
        input: Shared.aiAgents.ToolHelpers.fsHelper.createReadStream(
          targetURL,
          { encoding: 'utf8' }
        ),
      ...
      const fullContent = Shared.aiAgents.ToolHelpers.fsHelper.readFileSync(
        targetURL,
        'utf8'
      );
