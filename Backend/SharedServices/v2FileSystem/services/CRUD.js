...
import { containerVolumeRoot } from '../core/constants.js';
...
export async function readFileContent(filePath, asBuffer = false, options) {
  ...
  try {
    if (asBuffer) {
      ...
      return Services.v2Core.Helpers.Ok(bufferContent);
    } else {
      // Read as UTF-8 text or user specified encoding
      const textContent = await fsp.readFile(filePath, encoding);
      return Services.v2Core.Helpers.Ok(textContent);
    }
  } catch (error) {
    return Services.v2Core.Helpers.Err(`Error (readFileContent) : ${error}`);
  }
}
...
export async function getUpdateStatsFromUrl(url) {
  ...
  try {
    ...
    return Services.v2Core.Helpers.Err(error);
  }
}
...
export async function scanFolderRecursively(relativeFolderPath) {
  ...
  // Convert the Set back to an array before returning
  const directoryList = Array.from(directorySet);
  return Services.v2Core.Helpers.Ok({ directoryList, fileList }); // Ok({ directoryList, fileList }) =  [String], [String]
}
