...
export const IOStrategies = {
  TEXT: {
    ...
    write: async (folder, content, fileName) =>
      await saveFile(folder, content, fileName, { encoding: 'utf-8' }),
  },
  BINARY: {
    encoding: 'binary',
    read: async (filePath) => await readFileContent(filePath, true),
    write: async (folder, content, fileName) =>
      await saveFile(folder, content, fileName, { encoding: 'binary' }),
  },
  BASE64_IMAGE: {
    encoding: 'base64',
    read: async (filePath, mime) =>
      await readImageFileToBase64({ filePath, mimeType: mime }),
    write: async (folder, content, fileName) => {
      let prepImage =
        Services.aiAgents.AgentHelpers.prepareImageForSaving(content);
  ...
  AUDIO_PCM: {
    ...
    write: async (folder, content, fileName) => {
      let buffer =
        Services.aiAgents.AgentHelpers.processBase64Audio_ToWavBuffer(
          content,
