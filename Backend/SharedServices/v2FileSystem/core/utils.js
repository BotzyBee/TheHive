...
export async function readImageFileToBase64(params = {}) {
  ...
  if (fp == null || mt == null) {
    return Err(
      `Error (readImageFileToBase64) Paras needed filePath, mimeType. Provided: ${JSON.stringify(params)}`
    );
  }
  // read file to buffer
  ...
  if (call.isErr()) {
    return Err(
      `Error (readImageFileToBase64 -> readFileContent) : ${call.value}`
    );
  }
  // convert to base64
...
export async function readOfficeFileToString(params = {}) {
  ...
  if (call.isErr()) {
    return Services.v2Core.Helpers.Err(
      `Error (readOfficeFileToString -> readFileContent) : ${call.value}`
    );
  }
  // parse from buffer
  try {
    ...
    return Services.v2Core.Helpers.Err(
      `Error (readOfficeFileToString) : ${error}`
    );
  }
}
