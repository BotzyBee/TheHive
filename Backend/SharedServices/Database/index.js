...
import { dbURL, dbURL_Fallback, namespaceName, databaseName } from "../../constants";
...
let DATABASE_AGENT = null; // Global database object.
let su = new SharedUtils();

export class Database {
  constructor(){}
  getDatabaseAgent = getDbAgent;
  addDirectory = addDirectoryToDB;
  addFile = addFileToDB;
  addVector = addVectorToDB;
  getRecords = getRecords;
  searchVectorRecords = searchVectorRecords;
  getManagementData = getMgmtData;
  getRecordById = getRecordById;
  getDirsAndFilesFromUrl = getDirsAndFilesFromUrl;
  getDirsAndFilesRecursive = getDirsAndFilesRecursive;
  updateRecords = updateRecords;
  updateManagementData = updateMgmtData;
  deleteRecordsByField = deleteRecordsByField;
  deleteRecordsById = deleteRecordsById;
}
...
async function createDbAgent(offline = false, options = {}){
    ...
    try {
        ...
        if (optAttemptNumber == null || optAttemptNumber == undefined){
            ...
            if (dbAgent2.issu.result_ok()){ return su.result_ok(dbAgent2)}
        }
...
export async function initDatabaseConnection(offline = false){
  ...
  if(db.isOk()){ DATABASE_AGENT = db.value }
  return db;
}
