...
// [][] -- CREATE -- [][]
export async function addDirectoryToDB(dbAgent, URL, parentDirRef, lastUpdateMillis, metaObj){
    let encodedURL = encodeURIComponent(URL);
    try {
        const dirRef = su.longID("DIR");
        let result = await dbAgent.query(`INSERT INTO ${dirTableName} {
                DirRef: '${dirRef}',
                ParentDirRef: $pdr,
                Url: $u,
                LastUpdate: $lu,
...
export async function addFileToDB(dbAgent, dirRef, URL, fileType, lastUpdateMillis, metaObj){
    let encodedURL = encodeURIComponent(URL);
    try {
...
export async function addVectorToDB(dbAgent, tableName, dirRef, fileRef, URL, summary, vector) {
    let encodedURL = encodeURIComponent(URL);
    try {
        let result = await dbAgent.query(`
...
export async function searchVectorRecords(dbAgent, tableName, searchVector, limit = 5) {
    try {
...
export async function getMgmtData(dbAgent){
    try {
        ...
        let resLen = result[0].length ?? 0;
        if(resLen == 0 ){ return su.logAndErr("Error (getMgmtData) : DB returned no records") }
        return su.result_ok(result);
    } catch (error) {
...
export async function getDirsAndFilesFromUrl(dbAgent, Url){ 
    ...
    // catch error
    if (rootCheck.isErr()){ return su.logAndErr(`Error (getAllDirAndFilesFromUrl -> getRecords(1) ) : ${rootCheck.value}`) };
    // catch no root Dir
    let retLen = rootCheck.value[0].length ?? 0;
    if (retLen == 0) { return su.result_ok({ directoryList: [], fileList: [] }) }
    // root directory exists.. search for files and sub-directories
    let dirRef = rootCheck.value[0][0].DirRef; 
    let subFolders = await getRecords(dbAgent, dirTableName, "ParentDirRef", dirRef);
    // catch error
    if (subFolders.isErr()){ return su.logAndErr(`Error (getAllDirAndFilesFromUrl -> getRecords(2) ) : ${subFolders.value}`) };
    ...
    // catch error
    if (files.isErr()){ return su.logAndErr(`Error (getAllDirAndFilesFromUrl -> getRecords(3) ) : ${files.value}`) };
    // extract any files
...
export async function getDirsAndFilesRecursive(dbAgent, Url){  
    ...
    for(i=0; i<dirListLen; i++){
        const subCall = await getDirsAndFilesRecursive(dbAgent, directoryList[i].Url);
        if(subCall.isErr()){ return su.logAndErr(`Error - getDirsAndFilesRecursive -> Recursive call error ${subCall.value}`) }
...
// [][] -- UPDATE -- [][]
export async function updateRecords(dbAgent, tableName, searchField, searchTerm, updateData){
...
// [][] -- DELETE -- [][]
export async function deleteRecordsByField(dbAgent, tableName, fieldName, fieldValue) {
