...
import { getDbAgent } from '../Database/utils.js';
import { getRecords } from '../Database/CRUD.js';
...
export async function getToolsOrGuidesForTask(task, limit, modeTools = true){
    ...
    if(getDB.isErr()){
        return Err(`Error ( matchToolsToTask -> getDbAgent ) : ${getDB.value}`);
    }
    ...
    if( vec.isErr() ){ return Err(`Error ( matchToolsToTask -> generateEmbeddings ) : ${vec.value}`); }
    // Search the database
    let search = await searchVectorRecords(db, tableName, vec.value[0], limit);
    if( search.isErr() ){ return Err(`Error ( matchToolsToTask -> searchVectorRecords ) : ${search.value}`); }
    // remove embeddings
...
export async function getToolDetails(toolName){
    ...
    let getDB = await getDbAgent();
    if(getDB.isErr()){
        return Err(`Error ( getToolDetails -> getDbAgent ) : ${getDB.value}`);
    }
    const db = getDB.value;
    // Search DB
    let search = await getRecords(db, toolTableName, "ToolName", toolName);
    if( search.isErr() ){ return Err(`Error ( getToolDetails -> searchVectorRecords ) : ${search.value}`); }
    let resLen = search.value[0]?.length ?? 0;
    if(resLen == 0){ return Err(`Error ( getToolDetails -> searchVectorRecords 2 ) : DB returned no results `);}
    // Get tool object
    const fp = search.value[0][0].FilePath;
    let toolObj; 
    if(fp == builtInFilePath){
        // Built in tool
        toolObj = CoreTools.AgentCompatible[toolName].details;
    } else {
        // Plugin tool
    ...
    return Ok({filePath: fp, details: toolObj})
}