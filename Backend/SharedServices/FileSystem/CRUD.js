...
export async function saveFile(folderPath, fileContent, fileNameIncExt) {
    ...
    if (Buffer.isBuffer(fileContent) || fileContent instanceof Uint8Array) {
        ...
        dataToWrite = fileContent;
    } else if (Array.isArray(fileContent) || (typeof fileContent === 'object' && fileContent !== null)) {
    ...
    try {
        ...
        for (const item of items) {
            ...
            if (item.isDirectory()) {
                ...
                // catch error
                if(dirStats.isErr()){ return su.logAndErr(`Error - getFilesAndDirectoriesFromDir -> getUpdateStatsFromUrl(1) : ${dirStats.value}`)}
                ...
                if(fileStats.isErr()){ return su.logAndErr(`Error - getFilesAndDirectoriesFromDir -> getUpdateStatsFromUrl(2) : ${fileStats.value}`)}
                fileList.push({
                    fileName: item.name,
                    fileUrl: fileUrl,
                    fileType: fileType,
...
export async function scanFolderRecursively(relativeFolderPath) { 
    ...
    const targetDirectoryInContainer = path.join(containerVolumeRoot, relativeFolderPath);
    let fileList = [];
    let directorySet = new Set();
    try {
        // Read the contents of the directory
        const items = await fsp.readdir(targetDirectoryInContainer, { withFileTypes: true });
        for (const item of items) {
            ...
            if (item.isDirectory()) {
                ...
                if(recCall.isErr()){
                    return su.logAndErr(`Error (scanFolderRecursively) : Error scanning directory (recursive) ${fullPath} `);
                }
                const { directoryList: subfolderDirs, fileList: subfolderFiles } = recCall.value;
        ...
        return su.logAndErr(`Error (scanFolderRecursively) : Error scanning directory ${relativeFolderPath} : ${error} `);
    }
    // Convert the Set back to an array before returning
    const directoryList = Array.from(directorySet);
