...
export async function initModelRegistry() {
  ...
  if (dbAgentCall.isErr()) {
    return Services.v2Core.Helpers.Err(
      `Error (initRegistry -> getDbAgent) : ${dbAgentCall.value}`
    );
  }
  ...
  try {
    for (const modelConfig of MODEL_REGISTRY) {
      const { model, provider, capabilities, maxContext, quality } =
        modelConfig;

      // Check if the model already exists in the database by its 'model' identifier
      const existingModelResult = await getRecords(
        dbAgent,
        modelTableName,
        'model',
        model
      ...
      if (existingModelResult.isErr()) {
        return Services.v2Core.Helpers.Err(
          `Error (initRegistry -> getRecords) : ${existingModelResult.value}`
        );
      }
      ...
      if (existingModels.length === 0) {
        ...
        if (addResult.isErr()) {
          return Services.v2Core.Helpers.Err(
            `Error (initRegistry -> addModelToDB) : ${addResult.value}`
          );
        }
      } else {
    ...
    console.error(
      `Error during registry initialisation: ${error.message || error}`
    );
    return Services.v2Core.Helpers.Err(
      `Error (initRegistry) failed during registry initialisation: ${error.message || error}`
    );
  }
}
...
export async function getAllActiveModels() {
  try {
    ...
    if (dbAgentCall.isErr()) {
      return Services.v2Core.Helpers.Err(
        `Error (getAllActiveModels -> getDbAgent) : ${dbAgentCall.value}`
      );
    }
    const dbAgent = dbAgentCall.value;
    ...
    if (result.isErr()) {
      return Services.v2Core.Helpers.Err(
        `Error (getAllActiveModels) : ${result.value}`
      );
    }
    ...
    return Services.v2Core.Helpers.Err(
      `Error in getAllActiveModels: ${error.message || error}`
    );
  }
}
...
export async function getAllModels() {
  try {
    ...
    if (dbAgentCall.isErr()) {
      return Services.v2Core.Helpers.Err(
        `Error (getAllModels -> getDbAgent) : ${dbAgentCall.value}`
      );
    }
    const dbAgent = dbAgentCall.value;
    console.log('Fetching all active models from the registry...');
    ...
    if (result.isErr()) {
      return Services.v2Core.Helpers.Err(
        `Error (getAllModels) : ${result.value}`
      );
    }
    ...
    return Services.v2Core.Helpers.Err(
      `Error in getAllModels: ${error.message || error}`
    );
  }
}
