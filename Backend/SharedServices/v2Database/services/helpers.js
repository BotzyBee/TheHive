...
export async function getToolsOrGuidesForTask(task, limit, modeTools = true) {
  ...
  if (getDB.isErr()) {
    return Services.v2Core.Helpers.Err(
      `Error ( getToolsOrGuidesForTask -> getDbAgent ) : ${getDB.value}`
    );
  }
  ...
  if (vec.isErr()) {
    return Services.v2Core.Helpers.Err(
      `Error ( getToolsOrGuidesForTask -> generateEmbeddings ) : ${vec.value}`
    );
  }
  // Search the database
  ...
  if (search.isErr()) {
    return Services.v2Core.Helpers.Err(
      `Error ( getToolsOrGuidesForTask -> searchVectorRecords ) : ${JSON.stringify(search.value, null, 2)}`
    );
  }
  // remove embeddings
  let resLen = search.value[0].length ?? 0;
  for (let i = 0; i < resLen; i++) {
    search.value[0][i].FilePath = decodeURIComponent(
      search.value[0][i].FilePath
    );
...
export async function getToolDetails(toolName) {
  ...
  if (getDB.isErr()) {
    return Services.v2Core.Helpers.Err(
      `Error ( getToolDetails -> getDbAgent ) : ${getDB.value}`
    );
  }
  const db = getDB.value;
  // Search DB
  ...
  if (search.isErr()) {
    return Services.v2Core.Helpers.Err(
      `Error ( getToolDetails -> getRecords ) : ${search.value}`
    );
  }
  let resLen = search.value[0]?.length ?? 0;
  if (resLen == 0) {
    return Services.v2Core.Helpers.Err(
      `Error ( getToolDetails -> getRecords 2 ) : DB returned no results `
    );
  }
  // Get tool object
  ...
  if (fp == Services.fileSystem.Constants.builtInFilePath) {
    ...
    if (readFile) {
      ...
      return Services.v2Core.Helpers.Err(
        `Error ( getToolDetails -> import ) : Could not read file - ${fp}`
      );
    }
  }

  return Services.v2Core.Helpers.Ok({
    filePath: decodedFP ? decodedFP : fp,
    details: toolObj,
  });
}
