...
import { ModelTypes, AiQuality, AiProviders, MODEL_REGISTRY, PROVIDER_DISPATCH, DEFAULT_PROVIDER } from "../../constants.js";
...
export class AiCall {
    ...
    async generateText(systemMessage, contentMessage, options = {}) {
        ...
        return this.#dispatch(ModelTypes.text, systemMessage, contentMessage, options);
    }
    ...
    async webSearch(systemMessage, contentMessage, options = {}) {
        ...
        options.contextSize = tkns;
        options.useWeb = true;
        return this.#dispatch(ModelTypes.websearch, systemMessage, contentMessage, options);
    }
    ...
    #resolveModel(requiredCapability, options = {}) {
        const { model, provider, quality, structuredOutput, contextSize, randomModel } = options;
        ...
        if (model !== null && typeof model == "string") {
            ...
            if (!entry) {
                return su.logAndErr(`Error ( resolveModel ) : Unknown Model : ${model}`);
            }
            const missing = [...requiredCaps].filter(c => !entry.capabilities.includes(c));
            if (missing.length) {
                return su.logAndErr(`Error ( resolveModel ) : ${model} does not support ${missing.join(", ")}`)
            }
        ...
        if (capableCandidates.length === 0) {
            su.logAndErr(`Error ( resolveModel ) : No model found supporting capabilities: [${[...requiredCaps].join(", ")}].`)
        }
        // Find models with right sized max context 
        const ctxRequired = contextSize ?? 0;
        const contextSizeAndQualityApproved = capableCandidates.filter(m =>
            m.maxContext >= ctxRequired && m.quality == requestQuality
          );
        ...
        if(randomModel == true){
          let randomNum = Math.floor(Math.random() * contextSizeAndQualityApproved.length);
        ...
        const providerCandidates = capableCandidates.filter(m => m.provider === resolvedProvider);
        if (providerCandidates.length > 0) {
            // Path 3a: If contextSize provided, narrow to models whose maxContext fits
            if (contextSize != null) {
                const contextFit = providerCandidates.filter( m =>
                    m.maxContext >= contextSize
                );
                ...
                su.log(`WARN: ( AiCall -> resolveModel ) :  Provider "${resolvedProvider}" has no models with maxContext >= ${contextSize}.`+
            ...
            su.log(`WARN: ( resolveModel ) : Provider "${provider}" has no models supporting: ` +
        ...
        if (contextSize != null) {
            ...
            if (contextFit.length > 0) {
                ...
                return su.logAndErr(`Error ( resolveModel ) : No model across any provider has maxContext >= ${contextSize}. Unable to progress`);
            }
        }
    ...
    async #dispatch(capability, systemMessage, contentMessage, options) {
        ...
        if(entry.isErr()){ return su.result_err(`#dispatch -> `+entry.value) }
        const callFn   = this.#ProviderFunctions[entry.value.provider];
        if (!callFn) {
          return su.logAndErr(`Error : ( #dispatch ) No dispatch function registered for provider. ${entry.value.provider}`);
        }
