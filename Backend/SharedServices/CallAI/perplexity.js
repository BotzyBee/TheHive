 ...
 */
export async function callPerplexity(systemMessage, contentMessage, model, options = {}) {
    ...
    if (!model) {
        return su.logAndErr("Error (callPerplexity): No model provided in options.");
    }
    ...
    try {
        ...
        if (structuredOutput) {
            try {
                ...
                return su.logAndErr("Error (callPerplexity): Could not parse response to JSON.");
            }
        }
