 ...
 */
export async function callOpenAI(systemMessage, contentMessage, model, options = {}) {
    ...
    const chatModel = new ChatOpenAI({ model: modelChoice, openAIApiKey: apiKey });

    // Build message content - string for plain, array for multimodal
    let humanContent;
    if (hasImage) {
        humanContent = [
            ...(contentMessage.text ? [{ type: "text", text: contentMessage.text }] : []),
