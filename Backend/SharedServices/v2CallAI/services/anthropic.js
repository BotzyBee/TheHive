...
export async function callAnthropic(
  ...
  if (!capability) {
    return Services.v2Core.Helpers.Err(
      'Error (callAnthropic) : Capability param is missing or null. Ensure options.capability has valid ModelTypes'
    );
  }
  ...
  switch (capability) {
    ...
    case ModelTypes.image:
      return Services.v2Core.Helpers.Err(
        'Error (callAnthropic) : Anthropic does not have image capability.'
      );
    ...
    case ModelTypes.deepResearch:
      return Services.v2Core.Helpers.Err(
        'Error (callAnthropic) : Anthropic does not have deep research capability.'
      );

    case ModelTypes.websearch:
      return Services.v2Core.Helpers.Err(
        'Error (callAnthropic) : Anthropic does not have websearch capability.'
      );

    case ModelTypes.embedding:
      return Services.v2Core.Helpers.Err(
        'Error (callAnthropic) : Anthropic does not have embedding capability.'
      );

    case ModelTypes.textToSpeech:
      return Services.v2Core.Helpers.Err(
        'Error (callAnthropic) : Anthropic does not have text to speech capability.'
      );

    case ModelTypes.speechToText:
      return Services.v2Core.Helpers.Err(
        'Error (callAnthropic) : Anthropic does not have speech to text capability.'
      );

    case ModelTypes.maps:
      return Services.v2Core.Helpers.Err(
        'Error (callAnthropic) : Anthropic does not have maps capability.'
      );

    case ModelTypes.local:
      return Services.v2Core.Helpers.Err(
        'Error (callAnthropic) : Anthropic does not have local capability.'
      );

    default:
      return Services.v2Core.Helpers.Err(
        `Error (callAnthropic) "${capability}" not specifically handled.`
      );
  }
}
...
export async function generateText(
  ...
  if (!model) {
    return Services.v2Core.Helpers.Err(
      'Error (callAnthropic -> generateText): No model provided in options.'
    );
  }
  try {
    const params = {
    ...
    if (res.stop_reason === 'max_tokens') {
      return Services.v2Core.Helpers.Err(
        'Error: Claude hit the max_token limit. The JSON is incomplete.'
      );
    }
    ...
    if (structuredOutput) {
      try {
        ...
        console.error(
          'Failed to parse JSON. String preview:',
          textBlock.text.slice(-100)
        ); // See the end of the string
        return Services.v2Core.Helpers.Err(
          `JSON Syntax Error at position ${e.message}`
        );
      }
    } else {
    ...
    console.error(
      'Anthropic ERROR DEBUG :',
      JSON.stringify(schemaWithStrictness, null, 2)
    );
    return Services.v2Core.Helpers.Err(
      `Error (callAnthropic -> generateText): ${error}`
    );
  }
}
