...
export async function callKimi(
  ...
  if (!capability) {
    return Services.v2Core.Helpers.Err(
      'Error (callKimi) : Capability param is missing or null. Ensure options.capability has valid ModelTypes'
    );
  }
  ...
  switch (capability) {
    ...
    case ModelTypes.image:
      return Services.v2Core.Helpers.Err(
        'Error (callKimi) : Kimi image generation not implemented yet.'
      );

    case ModelTypes.websearch:
      return Services.v2Core.Helpers.Err(
        'Error (callKimi) : Kimi websearch not implemented yet.'
      );

    case ModelTypes.textToSpeech:
      return Services.v2Core.Helpers.Err(
        'Error (callKimi) : Kimi text to speech not implemented yet.'
      );

    case ModelTypes.speechToText:
      return Services.v2Core.Helpers.Err(
        'Error (callKimi) : Kimi speech to text not implemented yet.'
      );

    case ModelTypes.deepResearch:
      return Services.v2Core.Helpers.Err(
        'Error (callKimi) : Kimi deep research not implemented yet.'
      );

    case ModelTypes.maps:
      return Services.v2Core.Helpers.Err(
        'Error (callKimi) : Kimi maps capability not implemented yet.'
      );

    case ModelTypes.local:
      return Services.v2Core.Helpers.Err(
        'Error (callKimi) : Kimi local capability not implemented yet.'
      );

    default:
      return Services.v2Core.Helpers.Err(
        `Error (callKimi) : Capability "${capability}" not specifically handled for Kimi.`
      );
  }
}
...
export async function generateEmbeddings(model, options = {}) {
  ...
  try {
    ...
    return Services.v2Core.Helpers.Err(
      `Error (callKimi -> generateEmbeddings): ${error.message || error}`
    );
  }
}
...
export async function generateText(
  ...
  if (!model) {
    return Services.v2Core.Helpers.Err(
      'Error (callKimi -> generateText): No model provided.'
    );
  }
  ...
  try {
    ...
    return Services.v2Core.Helpers.Err(
      `Error (callKimi -> generateText): ${error.message || error}`
    );
  }
}
