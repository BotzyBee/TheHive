...
import { getRecords, getDirsAndFilesFromUrl, updateMgmtData, createDbAgent } from '../DB/CRUD.js';
...
export async function indexKnowledgebase(dbAgent){
    ...
    if (indexingActive == true){ return su.result_ok('Indexing is already in progress.')};
    // reset Index Stats
    kbTotal = 0;
    dbTotal = 0;
    totalJobCount = 0;
    ...
    if(rootUpdates.isErr()){ 
        ...
        return su.logAndErr(`Error indexKnowledgebase -> checkAndUpdateDirAndFiles(Root) ${rootUpdates.value}`)
    }
    ...
    while (i < furtherChecksNeeded){
        let nextUpdateDir = await checkAndUpdateDirAndFiles(dbAgent, furtherChecks[i]);
        if(nextUpdateDir.isErr()){ 
            indexingActive = false;
            return su.logAndErr(`Error indexKnowledgebase -> checkAndUpdateDirAndFiles(index ${i}) ${nextUpdateDir.value}`)
        }
    ...
    if (deleteActions.isErr()){ return su.logAndErr(`Error (indexKnowledgebase -> processDeleteJobs) : ${deleteActions}`)}

    // Update mgmt record with last change time for root directory
    let updateMgmtRec = await updateMgmtData(dbAgent, {lastIndexCheckMs: MsNow})
    if(updateMgmtRec.isErr()){
        indexingActive = false;
        su.logAndErr(`Error (indexKnowledgebase) - Could not update Mgmt Record : ${updateMgmtRec.value}`);
    }
...
async function checkAndUpdateDirAndFiles(dbAgent, Url){ // Url MUST be a Directory!!
    ...
    if(getRec.isErr()){ return su.logAndErr(`Error (checkAndUpdateDirAndFiles -> getRecords ) : ${getRec.value}`)}
    let getRecLen = getRec.value[0].length ?? 0;
    if (getRecLen == 0){
        return su.logAndErr(`Error (checkAndUpdateDirAndFiles -> getRecords ) : No record found for Url ${Url}. Unable to progress!`)
    }
    let thisDirRef = getRec.value[0][0].DirRef;
    // Get data from database
    let subDirsFiles = await getDirsAndFilesFromUrl(dbAgent, Url);
    // catch error
    if(subDirsFiles.isErr()){ return su.logAndErr(`Error (checkAndUpdateDirAndFiles -> getDirsAndFilesFromUrl ) : ${subDirsFiles.value}`)}
    ...
    // catch error
    if (kbFilDir.isErr()){ return su.logAndErr(`Error (checkAndUpdateDirAndFiles -> getFilesAndDirectoriesFromDir) : ${kbFilDir}`) }
    ...
    for(i=0; i<kbFileCount; i++){
            ...
            if(kbFiles[i].fileUrl == dbFiles[k].Url){
                ...
                if(kbFiles[i].updateMs != dbFiles[k].LastUpdate){
                   // Push UPDATE to changeList (Update DB)
                    changeList.push( 
                        new indexJob(dbFiles[k].Url)
                        .updateDbFile(dbAgent, {LastUpdate : kbFiles[i].updateMs}) 
                        );
        ...
        if(found == false){
            ...
            changeList.push( 
                new indexJob(kbFiles[i].fileUrl)
                .addFileToDB(dbAgent, thisDirRef, kbFiles[i].fileType, kbFiles[i].updateMs, {}) 
            );
            changeListReadable.push(`
                Add File to DB
    ...
    for(i=0; i<kbDirCount; i++){
        ...
        for(k=0; k<dbDirCount; k++){
            ...
            if(kbDirs[i].url == dbDirs[k].Url){
                ...
                if(kbDirs[i].updateMs != dbDirs[k].LastUpdate){
                    // Push UPDATE to changeList (Update DB)
                    changeList.push( 
                        new indexJob(kbDirs[i].url)
                        .updateDbDir(dbAgent, {LastUpdate : kbDirs[i].updateMs}) 
                    );
                    changeListReadable.push(`
                        Update DB Directory
                        Url: ${kbDirs[i].url}
        ...
        if(found == false){
            ...
            changeList.push( 
                new indexJob(kbDirs[i].url)
                .addDirToDB(dbAgent, thisDirRef, kbDirs[i].updateMs, {}) 
            );
            changeListReadable.push(`
                Add Dir to DB
                Url: ${kbDirs[i].url}
    ...
    // NOTE these are dealt with after all checkAndUpdateDirAndFiles calls have completed
     for(i=0; i<dbFileCount; i++){
        ...
        if(found == false){
            ...
            deleteJobs.push( 
                new indexJob(dbFiles[i].Url)
                .removeFileFromDB(dbAgent)
            );
    ...
    // Check for deleted Directories
     for(i=0; i<dbDirCount; i++){
        ...
        if(found == false){
            ...
            // Push delete to deleteJobs (Remove from DB)
            deleteJobs.push( 
                new indexJob(dbDirs[i].Url)
                .removeDirFromDB(dbAgent)
            );
            deleteJobsReadable.push(`
                Remove Dir from DB
                Url: ${dbDirs[i].Url}
