...
export class indexJob {
    ...
    async addFileToDB(dbAgent, dirRef, fileType, updateMillis, metaObj) {
       let res = await db.addFile(dbAgent, dirRef, this.url, fileType, updateMillis, metaObj);
       if(res.isErr()){ 
    ...
    async addDirToDB(dbAgent, parentDirRef, updateMillis, metaObj) {
       let res = await db.addDirectory(dbAgent, this.url, parentDirRef, updateMillis, metaObj);
       if(res.isErr()){
    ...
    async updateDbFile(dbAgent, updateData){
        ...
        let res = await db.updateRecords(dbAgent, fileTableName, "Url", this.url, updateData);
        if(res.isErr()){
            this.attemptNumber += 1;
    ...
    async updateDbDir(dbAgent, updateData){
        // upadate data = object of keys/ values to be updated 
        // eg {LastUpdate: 123, Meta: { tags: ["cheese", "potato"] } }
        let res = await db.updateRecords(dbAgent, dirTableName, "Url", this.url, updateData);
        if(res.isErr()){
            this.attemptNumber += 1;
    ...
    async removeFileFromDB(dbAgent){
        let res = await db.deleteRecordsByField(dbAgent, fileTableName, "Url", this.url );
        if(res.isErr()){
            this.attemptNumber += 1;
    ...
    async removeDirFromDB(dbAgent){
        ...
        for(i=0; i<allDirsLen; i++){
            let allDirRes = await db.deleteRecordsByField(dbAgent, dirTableName, "Url", allSubs.value.directoryList[i].Url );
            if(allDirRes.isErr()){
                this.attemptNumber += 1;
        ...
        for(i=0; i<allFileLen; i++){
            let allFileRes = await db.deleteRecordsByField(dbAgent, fileTableName, "Url", allSubs.value.fileList[i].Url );
            if(allFileRes.isErr()){
                this.attemptNumber += 1;
        ...
        // Delete Root Dir
        let res = await db.deleteRecordsByField(dbAgent, dirTableName, "Url", this.url );
        if(res.isErr()){
            this.attemptNumber += 1;
